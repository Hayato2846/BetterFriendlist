...
local function fallback_encode(s)
    ...
    for i = 1, #s, 1 do
        ...
        if y == 3 then
            ...
            r = r .. string.sub(b64chars, math.floor(((b[1] % 4) * 16) + (b[2] / 16)) + 1, math.floor(((b[1] % 4) * 16) + (b[2] / 16)) + 1)
            r = r .. string.sub(b64chars, math.floor(((b[2] % 16) * 4) + (b[3] / 64)) + 1, math.floor(((b[2] % 16) * 4) + (b[3] / 64)) + 1)
...
local function fallback_decode(s)
    ...
    for i = 1, #s, 1 do
        ...
        if c >= 65 and c <= 90 then b[z+1] = c - 65
        elseif c >= 97 and c <= 122 then b[z+1] = c - 71
        elseif c >= 48 and c <= 57 then b[z+1] = c + 4
        elseif c == 43 then b[z+1] = 62
        elseif c == 47 then b[z+1] = 63
        elseif c == 61 then break end
        z = z + 1
        if z == 4 then
