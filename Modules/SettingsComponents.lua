...
local function GetDropdownFontObject(fontPath, sizeOffset)
	...
	if not fontObject then
		...
		local _, baseSize, baseFlags = baseFont:GetFont()
...
function Components:CreateColorPicker(parent, labelText, initialColor, callback)
	...
	colorBorder:SetColorTexture(0, 0, 0, 0)
...
-- ========================================
function Components:CreateSliderWithColorPicker(
	parent,
	labelText,
	min,
	max,
	...
	colorSwatch:SetColorTexture(
		holder.currentColor.r,
		holder.currentColor.g,
		holder.currentColor.b,
		holder.currentColor.a or 1
	)
...
-- ========================================
function Components:CreateListItem(
	parent,
	itemText,
	orderIndex,
	onDragStart,
	...
	local function ResolveAlpha(color, defaultAlpha)
		if not color then
			return defaultAlpha
		end
	...
	if onDragStart and onDragStop then
		holder:SetScript("OnDragStart", function(self)
			...
			if initialColors and initialColors.fallback then
				ghost.stripe:SetColorTexture(
					initialColors.fallback.r,
					initialColors.fallback.g,
	...
	holder.orderText:SetWidth(26) -- Reserve space for up to 3 digits
	holder.orderText:SetJustifyH("RIGHT")
	...
	if onArrowColor then
		...
		holder.arrowColorBorder:SetColorTexture(0, 0, 0, 0)
		...
		if initialColors and initialColors.arrow then
			local arrowAlpha = ResolveAlpha(initialColors.arrow, 1)
			holder.arrowColorSwatch:SetColorTexture(
				initialColors.arrow.r,
				initialColors.arrow.g,
				initialColors.arrow.b,
				arrowAlpha
			)
		elseif initialColors and initialColors.fallback then
			local fallbackAlpha = ResolveAlpha(initialColors.fallback, 1)
			holder.arrowColorSwatch:SetColorTexture(
				initialColors.fallback.r,
				initialColors.fallback.g,
				initialColors.fallback.b,
		...
		holder.arrowColorButton:SetScript("OnClick", function(self, button)
			if button == "RightButton" then
				...
				if onArrowColor then
					onArrowColor(holder.arrowColorSwatch)
				end
			end
		end)
	...
	if onCountColor then
		...
		holder.countColorBorder:SetColorTexture(0, 0, 0, 0)
		...
		if initialColors and initialColors.count then
			local countAlpha = ResolveAlpha(initialColors.count, 1)
			holder.countColorSwatch:SetColorTexture(
				initialColors.count.r,
				initialColors.count.g,
				initialColors.count.b,
				countAlpha
			)
		elseif initialColors and initialColors.fallback then
			local fallbackAlpha = ResolveAlpha(initialColors.fallback, 1)
		...
		holder.countColorButton:SetScript("OnClick", function(self, button)
			if button == "RightButton" then
				...
				if onCountColor then
					onCountColor(holder.countColorSwatch)
				end
			end
		end)
	...
	holder.colorBorder:SetColorTexture(0, 0, 0, 0)
...
function Components:CreateDoubleCheckbox(parent, leftData, rightData)
	...
	if leftData then
		...
		if leftCheck.Text then
			leftCheck.Text:SetText("")
		end
		if leftCheck.SetText then
			leftCheck:SetText("")
		end
		...
		leftCheck:SetScript("OnClick", function(self)
			if leftData.callback then
				leftData.callback(self:GetChecked())
			end
		end)
	...
	if rightData then
		...
		if rightCheck.Text then
			rightCheck.Text:SetText("")
		end
		if rightCheck.SetText then
			rightCheck:SetText("")
		end
		...
		rightCheck:SetScript("OnClick", function(self)
			if rightData.callback then
				rightData.callback(self:GetChecked())
			end
		end)
...
-- ========================================
-- CHECKBOX + DROPDOWN ROW (Left Checkbox, Right Dropdown)
-- ========================================
function Components:CreateCheckboxDropdown(parent, checkboxData, dropdownData)
	local holder = CreateFrame("Frame", nil, parent)
	holder:SetHeight(COMPONENT_HEIGHT + 8)
	holder:SetPoint("LEFT", PADDING_LEFT, 0)
	holder:SetPoint("RIGHT", -PADDING_RIGHT, 0)
	...
	if checkboxData then
		leftLabel = holder:CreateFontString(nil, "ARTWORK", "BetterFriendlistFontHighlight")
		leftLabel:SetJustifyH("LEFT")
		leftLabel:SetText(checkboxData.label)
		...
		leftCheck:SetChecked(checkboxData.initialValue)
		leftCheck:SetScript("OnClick", function(self)
			if checkboxData.callback then
				checkboxData.callback(self:GetChecked())
			end
		end)
		leftCheck:SetScript("OnEnter", function(self)
	...
	-- Right: Label + Dropdown (optional)
	local rightLabel, dropdown
	if dropdownData then
		...
		if BFL.IsClassic or not BFL.HasModernMenu then
			...
			if dropdownData.entries and dropdownData.entries.labels and dropdownData.entries.values then
				...
				local entryFontPaths = dropdownData.entries.fontPaths
				local useCheckboxes = dropdownData.entries.useCheckboxes
				local dropdownText = dropdown.Text
				local defaultFontObject = GetDefaultDropdownFontObject(2)
	...
	holder:SetScript("OnSizeChanged", function(self, width, height)
		...
		if rightLabel and dropdown then
			...
			rightLabel:ClearAllPoints()
			rightLabel:SetPoint("LEFT", self, "LEFT", (width / 2) + (CONTROL_GAP * 2), 0)
			rightLabel:SetPoint("RIGHT", dropdown, "LEFT", -CONTROL_GAP, 0)
		end
	end)
...
function Components:CreateLabel(parent, text, isSmall, color)
	local label = parent:CreateFontString(
		nil,
		"ARTWORK",
		isSmall and "BetterFriendlistFontHighlightSmall" or "BetterFriendlistFontHighlight"
	)
