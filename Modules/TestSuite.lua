...
function TestSuite:ShowStatus()
	...
	print("  Preview Mode:       " .. (BFL:GetModule("PreviewMode") and BFL:GetModule("PreviewMode").enabled and "|cff00ff00Active|r" or "|cff888888Inactive|r"))
	print("")
...
function TestSuite:HandleScenarioCommand(args)
		...
		if ScenarioManager then
			...
			if #presets > 0 then
				...
				for _, preset in ipairs(presets) do
					print(string.format("  * |cffffffff%s|r - %s (%d friends)", 
						preset.name, preset.description, preset.friendCount or 0))
				end
				print("")
			end
			...
			if #saved > 0 then
				...
				for _, scenario in ipairs(saved) do
					...
					print(string.format("  * |cffffffff%s|r (%d friends, saved %s)", 
						scenario.name, scenario.friendCount or 0, savedAt))
				end
				print("")
			else
	...
	elseif cmd == "import" then
		...
		self.Reporter:Info("Import: Paste scenario string (BFL_SCENARIO_V1:...) to /bfl test scenario import <name> <string>")
...
local function RegisterBuiltInTests()
	...
	TS:RegisterTest("ui", "Settings_GroupsTab_ListItems", {
		...
		action = function(V)
			local Settings = BFL:GetModule("Settings")
			if not Settings then V:Skip("Settings not loaded") return end
			Settings:Show()
			Settings:RefreshGroupsTab()
			
			local frame = _G.BetterFriendlistSettingsFrame
			local tab = frame and frame.ContentScrollFrame and frame.ContentScrollFrame.Content and frame.ContentScrollFrame.Content.GroupsTab
			...
			V:Assert(firstItem.renameButton and firstItem.renameButton:GetScript("OnClick") ~= nil,
	...
	TS:RegisterTest("ui", "Settings_GroupsTab_Drag_Smoke", {
		...
		action = function(V)
			...
			local frame = _G.BetterFriendlistSettingsFrame
			local tab = frame and frame.ContentScrollFrame and frame.ContentScrollFrame.Content and frame.ContentScrollFrame.Content.GroupsTab
			if not tab then V:Skip("GroupsTab not available") return end
	...
	TS:RegisterTest("ui", "FriendsTab_HasScrollBox", {
		...
		condition = function() return BFL.IsRetail end,
		setup = function()
			if BetterFriendsFrame and not BetterFriendsFrame:IsShown() then
				ToggleBetterFriendsFrame()
			end
	...
	TS:RegisterTest("data", "Database_Migration_NameDisplayFormat", {
		...
		action = function(V)
			WithTemporaryDatabase({
				...
				V:AssertEqual(tempDB.nameDisplayFormat, "%note% (%nickname%)",
	...
	TS:RegisterTest("data", "Database_Migration_DefaultFrameWidth", {
		...
		action = function(V)
			WithTemporaryDatabase({
				...
				V:AssertEqual(tempDB.defaultFrameWidth, 380,
					"defaultFrameWidth should be migrated to 380")
			end)
		end,
	})
	...
	TS:RegisterTest("data", "Database_Migration_ColorTables", {
		...
		action = function(V)
			...
			WithTemporaryDatabase({
				...
				V:Assert(type(tempDB.fontColorFriendName) == "table" and tempDB.fontColorFriendName.r ~= nil,
					"fontColorFriendName should be repaired")
				V:Assert(type(tempDB.fontColorFriendInfo) == "table" and tempDB.fontColorFriendInfo.r ~= nil,
					"fontColorFriendInfo should be repaired")
				V:Assert(tempDB.groupCountColors ~= tempDB.groupArrowColors,
	...
	TS:RegisterTest("classic", "Classic_NoRecentAllies", {
		...
		action = function(V)
			...
			V:Assert(BFL.HasRecentAllies == compatAvailable,
				"HasRecentAllies should match API availability")
		end,
	})
	...
	TS:RegisterTest("integration", "AllModules_Loaded", {
		...
		action = function(V)
			local requiredModules = {
				"DB", "Groups", "FriendsList", "Settings", "Changelog",
				"MenuSystem", "Dialogs", "QuickFilters", "WhoFrame",
			}
	...
	TS:RegisterTest("integration", "Scenario_Load_Stress200", {
		...
		action = function(V)
			local ScenarioManager = BFL.ScenarioManager
			local FriendsList = BFL:GetModule("FriendsList")
			if not ScenarioManager then V:Skip("ScenarioManager not available") return end
			if not FriendsList then V:Skip("FriendsList not loaded") return end
			
			local ok = ScenarioManager:Load("stress_200")
			V:Assert(ok == true, "Scenario stress_200 should load")
			
			FriendsList:UpdateFriendsList()
			V:Assert(FriendsList.friendsList and #FriendsList.friendsList > 0,
	...
	TS:RegisterTest("integration", "PreviewMode_RestorePersistedState", {
		...
		action = function(V)
			local PreviewMode = BFL:GetModule("PreviewMode")
			if not PreviewMode then V:Skip("PreviewMode not available") return end
			if not BetterFriendlistDB then V:Skip("DB not initialized") return end
	...
	TS:RegisterTest("integration", "PreviewMode_Enable_Disable_RoundTrip", {
		...
		action = function(V)
			local PreviewMode = BFL:GetModule("PreviewMode")
			if not PreviewMode then V:Skip("PreviewMode not available") return end
			if PreviewMode.enabled then V:Skip("PreviewMode already enabled") return end
			if not BetterFriendlistDB then V:Skip("DB not initialized") return end
	...
	TS:RegisterTest("integration", "QuickJoin_MockGroup_Structure", {
		...
		action = function(V)
			local QuickJoin = BFL:GetModule("QuickJoin")
			if not QuickJoin then V:Skip("QuickJoin not loaded") return end
	...
	TS:RegisterTest("integration", "RaidFrame_ClearMockData", {
		...
		action = function(V)
			local RaidFrame = BFL:GetModule("RaidFrame")
			if not RaidFrame then V:Skip("RaidFrame not loaded") return end
	...
	TS:RegisterTest("integration", "WhoFrame_Update_NoError", {
		...
		action = function(V)
			...
			if not WhoFrame then V:Skip("WhoFrame not loaded") return end
	...
	TS:RegisterTest("integration", "StreamerMode_Toggle", {
		...
		action = function(V)
			...
			if not StreamerMode then V:Skip("StreamerMode not loaded") return end
			if not BetterFriendlistDB then V:Skip("DB not initialized") return end
	...
	TS:RegisterTest("integration", "Broker_Update_NoError", {
		...
		action = function(V)
			...
			if not Broker or not Broker.UpdateBrokerText then V:Skip("Broker not loaded") return end
	...
	TS:RegisterTest("integration", "MenuSystem_WhoFlag", {
		...
		action = function(V)
			...
			if not MenuSystem then V:Skip("MenuSystem not loaded") return end
	...
	TS:RegisterTest("integration", "Dialogs_Register", {
		...
		action = function(V)
			...
			if not Dialogs then V:Skip("Dialogs not loaded") return end
			Dialogs:RegisterDialogs()
			V:Assert(StaticPopupDialogs["BETTER_FRIENDLIST_CREATE_GROUP"] ~= nil,
				"Create group dialog should be registered")
			V:Assert(StaticPopupDialogs["BETTER_FRIENDLIST_RENAME_GROUP"] ~= nil,
	...
	TS:RegisterTest("integration", "RAF_Guard_NoError", {
		...
		action = function(V)
			...
			if not RAF then V:Skip("RAF not loaded") return end
			if BFL.HasRAF then V:Skip("RAF available; guard test not applicable") return end
	...
	TS:RegisterTest("integration", "RecentAllies_Unavailable_Message", {
		...
		action = function(V)
			...
			if not RecentAllies then V:Skip("RecentAllies not loaded") return end
			if BFL.HasRecentAllies then V:Skip("RecentAllies available; guard test not applicable") return end
	...
	TS:RegisterTest("integration", "Compat_GlobalIgnoreList_Setup", {
		...
		action = function(V)
			...
			if not Compat then V:Skip("Compat module not loaded") return end
			Compat:Setup()
	...
	TS:RegisterTest("integration", "Retail_ApiGuard_QuickJoin_GetAllGroups_Nil", {
		...
		action = function(V)
			local QuickJoin = BFL:GetModule("QuickJoin")
			if not QuickJoin then V:Skip("QuickJoin not loaded") return end
			if not C_SocialQueue or not C_SocialQueue.GetAllGroups then V:Skip("C_SocialQueue not available") return end
			local originalGetAllGroups = C_SocialQueue.GetAllGroups
			C_SocialQueue.GetAllGroups = function() return nil end
	...
	TS:RegisterTest("settings", "CacheVersion_Increments_On_Set", {
		...
		action = function(V)
			...
			V:Assert(versionAfter > versionBefore, 
				"SettingsVersion should increment after DB:Set() (was " .. versionBefore .. ", now " .. versionAfter .. ")")
	...
	TS:RegisterTest("settings", "FriendsList_Cache_Invalidation", {
		...
		action = function(V)
			...
			V:Assert(BFL.SettingsVersion > cachedVersion, 
				"SettingsVersion should be higher than cached version")
			
			-- UpdateSettingsCache should detect the change
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCacheVersion == BFL.SettingsVersion,
	...
	TS:RegisterTest("settings", "CompactMode_Effect", {
		...
		action = function(V)
			...
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.compactMode == true, 
				"Cache should reflect compactMode=true")
			
			-- Set to false
			DB:Set("compactMode", false)
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.compactMode == false, 
				"Cache should reflect compactMode=false")
	...
	TS:RegisterTest("settings", "HideEmptyGroups_Effect", {
		...
		action = function(V)
			...
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.hideEmptyGroups == true, 
				"Cache should reflect hideEmptyGroups=true")
			
			DB:Set("hideEmptyGroups", false)
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.hideEmptyGroups == false, 
				"Cache should reflect hideEmptyGroups=false")
	...
	TS:RegisterTest("settings", "FavoritesHidden_FallsToNoGroup", {
		...
		action = function(V)
			...
			local MockDataProvider = BFL.MockDataProvider
			if not FriendsList then V:Skip("FriendsList not loaded") return end
			if not DB then V:Skip("DB not loaded") return end
			if not MockDataProvider then V:Skip("MockDataProvider not available") return end
	...
	TS:RegisterTest("sort", "SortByName_Alphabetical", {
		...
		action = function(V)
			...
			V:Assert(FriendsList.friendsList[1]._sort_name == "alpha", 
				"First should be 'alpha', got '" .. tostring(FriendsList.friendsList[1]._sort_name) .. "'")
			V:Assert(FriendsList.friendsList[2]._sort_name == "mike", 
				"Second should be 'mike', got '" .. tostring(FriendsList.friendsList[2]._sort_name) .. "'")
			V:Assert(FriendsList.friendsList[3]._sort_name == "zephyr", 
	...
	TS:RegisterTest("sort", "SortByStatus_OnlineFirst", {
		...
		action = function(V)
			...
			V:Assert(FriendsList.friendsList[1]._sort_isOnline == true, 
				"First friend should be online")
			V:Assert(FriendsList.friendsList[2]._sort_isOnline == false, 
				"Second friend should be offline")
	...
	TS:RegisterTest("sort", "SortByLevel_Descending", {
		...
		action = function(V)
			...
			local testFriends = {
				{ _sort_name = "low", _sort_level = 10, _sort_isOnline = true, _sort_status = 1, index = 1, isFavorite = false },
				{ _sort_name = "max", _sort_level = 80, _sort_isOnline = true, _sort_status = 1, index = 2, isFavorite = false },
				{ _sort_name = "mid", _sort_level = 50, _sort_isOnline = true, _sort_status = 1, index = 3, isFavorite = false },
			}
	...
	TS:RegisterTest("sort", "Favorites_AlwaysFirst", {
		...
		action = function(V)
			...
			V:Assert(FriendsList.friendsList[1].isFavorite == true, 
				"First friend should be favorite")
	...
	TS:RegisterTest("filter", "OnlineFilter_ShowsOnlyOnline", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(onlineFriend) == true, 
				"Online friend should pass 'online' filter")
			V:Assert(FriendsList:PassesFilters(offlineFriend) == false, 
	...
	TS:RegisterTest("filter", "OfflineFilter_ShowsOnlyOffline", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(onlineFriend) == false, 
				"Online friend should NOT pass 'offline' filter")
			V:Assert(FriendsList:PassesFilters(offlineFriend) == true, 
				"Offline friend should pass 'offline' filter")
	...
	TS:RegisterTest("filter", "WoWFilter_RequiresWoWClient", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(wowFriend) == true, 
				"BNet friend in WoW should pass 'wow' filter")
			V:Assert(FriendsList:PassesFilters(owFriend) == false, 
				"BNet friend in other game should NOT pass 'wow' filter")
			V:Assert(FriendsList:PassesFilters(offlineFriend) == false, 
				"Offline BNet friend should NOT pass 'wow' filter")
			V:Assert(FriendsList:PassesFilters(wowOnlyFriend) == true, 
				"WoW-only friend should pass 'wow' filter")
	...
	TS:RegisterTest("filter", "SearchFilter_MatchesName", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(friend) == true, 
				"Friend should pass when search matches name")
			
			-- Search that doesn't match
			FriendsList.searchText = "xyz123"
			V:Assert(FriendsList:PassesFilters(friend) == false, 
				"Friend should NOT pass when search doesn't match")
	...
	TS:RegisterTest("filter", "SearchFilter_MatchesNote", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(friend) == true, 
				"Friend should pass when search matches note")
	...
	TS:RegisterTest("filter", "HideAFK_FiltersAFKFriends", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(availableFriend) == true, 
				"Available friend should pass 'hideafk' filter")
			V:Assert(FriendsList:PassesFilters(afkFriend) == false, 
				"AFK friend should NOT pass 'hideafk' filter")
			V:Assert(FriendsList:PassesFilters(dndFriend) == false, 
				"DND friend should NOT pass 'hideafk' filter")
	...
	TS:RegisterTest("bugs", "Nickname_Persistence", {
		...
		action = function(V)
			...
			V:Assert(retrieved == testNickname, 
				"Nickname should be retrievable after setting (expected '" .. testNickname .. "', got '" .. tostring(retrieved) .. "')")
			
			-- Clear nickname
			DB:SetNickname(testUID, nil)
			local cleared = DB:GetNickname(testUID)
			V:Assert(cleared == nil, 
				"Nickname should be nil after clearing")
		end,
	})
	...
	TS:RegisterTest("bugs", "GroupAssignment_Persistence", {
		...
		action = function(V)
			local DB = BFL:GetModule("DB")
			local Groups = BFL:GetModule("Groups")
			if not Groups then V:Skip("Groups module not loaded") return end
			...
			V:Assert(DB:IsFriendInGroup(testUID, groupId) == true, 
				"Friend should be in group after adding")
			
			-- Remove friend from group
			DB:RemoveFriendFromGroup(testUID, groupId)
			V:Assert(DB:IsFriendInGroup(testUID, groupId) == false, 
				"Friend should not be in group after removing")
	...
	TS:RegisterTest("groups", "GroupRename_Works", {
		...
		action = function(V)
			...
			if not success then V:Skip("Could not create test group") return end
	...
	TS:RegisterTest("events", "HiddenFrame_MarksDirty", {
		...
		action = function(V)
			...
			V:Assert(updateCalled == false, 
				"OnFriendListUpdate should NOT call UpdateFriendsList when frame is hidden")
		end,
	})
	...
	TS:RegisterTest("events", "GroupChange_TriggersVersionIncrement", {
		...
		action = function(V)
			local DB = BFL:GetModule("DB")
			local Groups = BFL:GetModule("Groups")
			if not DB then V:Skip("DB module not loaded") return end
			if not Groups then V:Skip("Groups module not loaded") return end
			...
			V:Assert(versionAfterAdd > versionBefore, 
				"SettingsVersion should increment after AddFriendToGroup")
			
			-- Remove friend from group
			DB:RemoveFriendFromGroup(testUID, groupId)
			
			local versionAfterRemove = BFL.SettingsVersion or 1
			V:Assert(versionAfterRemove > versionAfterAdd, 
	...
	TS:RegisterTest("events", "EventCallback_Registration", {
		...
		action = function(V)
			...
			V:Assert(BFL.FireEventCallbacks ~= nil, 
				"BFL.FireEventCallbacks must exist")
			V:Assert(BFL.RegisterEventCallback ~= nil, 
				"BFL.RegisterEventCallback must exist")
			...
			V:Assert(hasAnyCallbacks, 
				"At least one event must have registered callbacks")
		end,
	})
	...
	TS:RegisterTest("events", "FireEventCallbacks_Works", {
		...
		action = function(V)
			...
			V:Assert(callbackFired == true, 
				"Callback should have been fired")
			V:Assert(receivedArgs and receivedArgs[1] == "test1", 
	...
	TS:RegisterTest("updates", "ForceRefresh_UpdatesList", {
		...
		action = function(V)
			local FriendsList = BFL:GetModule("FriendsList")
			if not FriendsList then V:Skip("FriendsList not loaded") return end
			if not BFL.ForceRefreshFriendsList then V:Skip("ForceRefreshFriendsList not defined") return end
			...
			V:Assert(updateCalled == true, 
				"ForceRefreshFriendsList should trigger UpdateFriendsList")
		end,
	})
	...
	TS:RegisterTest("updates", "FilterChange_RefreshesUI", {
		...
		action = function(V)
			...
			BFL.ForceRefreshFriendsList = function(...)
				refreshCalled = true
				if originalRefresh then return originalRefresh(...) end
			end
			...
			V:Assert(refreshCalled == true, 
				"SetFilterMode should trigger ForceRefreshFriendsList")
		end,
	})
	...
	TS:RegisterTest("updates", "SortChange_RefreshesUI", {
		...
		action = function(V)
			...
			V:Assert(refreshCalled == true, 
				"SetSortMode should trigger ForceRefreshFriendsList")
		end,
	})
	...
	TS:RegisterTest("updates", "SearchText_FiltersCorrectly", {
		...
		action = function(V)
			...
			V:Assert(FriendsList:PassesFilters(nonMatchingFriend) == false, 
			...
			V:Assert(FriendsList:PassesFilters(matchingFriend) == true, 
	...
	TS:RegisterTest("updates", "NicknameChange_InvalidatesCache", {
		...
		action = function(V)
			...
			V:Assert(versionAfter > versionBefore, 
				"NicknameCacheVersion should increment after SetNickname")
	...
	TS:RegisterTest("render", "FriendsList_NotNil_After_Update", {
		...
		action = function(V)
			...
			V:Assert(FriendsList.friendsList ~= nil, 
				"friendsList should not be nil after UpdateFriendsList")
			V:Assert(type(FriendsList.friendsList) == "table", 
				"friendsList should be a table")
		end,
	})
		...
		action = function(V)
			...
			for groupId, groupData in pairs(grouped) do
				if type(groupData) == "table" then
					V:Assert(type(groupId) == "string", 
						"Group ID should be string, got " .. type(groupId))
				end
			end
		end,
	})
	...
	TS:RegisterTest("render", "BuiltInRename_UpdatesHeader", {
		...
		action = function(V)
			...
			local MockDataProvider = BFL.MockDataProvider
			if not FriendsList then V:Skip("FriendsList not loaded") return end
			if not Groups then V:Skip("Groups not loaded") return end
			if not MockDataProvider then V:Skip("MockDataProvider not available") return end
	...
	TS:RegisterTest("render", "SettingsCache_Not_Empty", {
		...
		action = function(V)
			...
			V:Assert(cache.nameDisplayFormat ~= nil, 
				"settingsCache.nameDisplayFormat should not be nil")
			V:Assert(cache.compactMode ~= nil or cache.compactMode == false, 
	...
	TS:RegisterTest("issues", "Issue41_ShowGroupArrow_Setting", {
		...
		action = function(V)
			...
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.showGroupArrow == false, 
				"showGroupArrow=false must be in cache")
			
			-- Set to true
			DB:Set("showGroupArrow", true)
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.showGroupArrow == true, 
				"showGroupArrow=true must be in cache")
	...
	TS:RegisterTest("issues", "Issue36_ShowFactionIcons_Setting", {
		...
		action = function(V)
			...
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.showFactionIcons == true, 
				"showFactionIcons=true must be cached")
			
			-- Disable faction icons
			DB:Set("showFactionIcons", false)
			FriendsList:UpdateSettingsCache()
			V:Assert(FriendsList.settingsCache.showFactionIcons == false, 
				"showFactionIcons=false must be cached")
	...
	TS:RegisterTest("issues", "Issue25_GlobalSync_SkipsSelf", {
		...
		action = function(V)
			...
			V:Assert(shouldAdd == false, 
				"GlobalSync should not try to add player (" .. fullName .. ") as friend")
		end,
	})
	...
	TS:RegisterTest("issues", "Issue21_Combat_NoProtectedCalls", {
		...
		action = function(V)
			...
			V:Assert(type(InCombatLockdown) == "function", 
				"InCombatLockdown must be a function for combat protection")
		end,
	})
	...
	TS:RegisterTest("issues", "Issue18_NoAutoOpen_OnReload", {
		...
		action = function(V)
			...
			V:Assert(forceOpen == nil or forceOpen == false, 
				"forceOpenOnLogin should not be set/true")
			V:Assert(autoShow == nil or autoShow == false, 
				"autoShowOnLoad should not be set/true")
		end,
	})
	...
	TS:RegisterTest("issues", "Issue15_StrsplitNilProtection", {
		...
		action = function(V)
			...
			if FriendsList.GetFriendUID then
				...
				V:Assert(result2 == nil or type(result2) == "string", 
					"GetFriendUID with nil name should not error")
				
				-- Test with empty friend
				local result3 = FriendsList:GetFriendUID({})
				V:Assert(result3 == nil or type(result3) == "string", 
					"GetFriendUID({}) should not error")
			else
	...
	TS:RegisterTest("issues", "Issue26_ElvUISkin_SafeAccess", {
		...
		action = function(V)
			...
			if not ElvUISkin then V:Skip("ElvUISkin not loaded") return end
	...
	TS:RegisterTest("issues", "Issue14_DefaultColorMatches", {
		...
		action = function(V)
			...
			V:Assert(defaultColor.r and math.abs(defaultColor.r - expectedR) < tolerance, 
				"Default red should be ~0.51, got " .. tostring(defaultColor.r))
			V:Assert(defaultColor.g and math.abs(defaultColor.g - expectedG) < tolerance, 
				"Default green should be ~0.773, got " .. tostring(defaultColor.g))
			V:Assert(defaultColor.b and math.abs(defaultColor.b - expectedB) < tolerance, 
	...
	TS:RegisterTest("issues", "Issue27_NameNotOverTruncated", {
		...
		action = function(V)
			...
			if FriendsList.GetDisplayName then
				...
				V:Assert(displayName ~= nil and displayName ~= "", 
					"GetDisplayName should return non-empty string")
			else
	...
	TS:RegisterTest("issues", "Issue13_JoinButton_Exists", {
		...
		action = function(V)
			...
			if BFL.HasQuickJoin then
				V:Assert(BFL.HasQuickJoin == true or BFL.HasQuickJoin == false, 
					"HasQuickJoin flag should be defined")
			end
			
			-- Check QuickJoin module exists on retail
			if BFL.IsRetail and BFL:GetModule("QuickJoin") then
				V:Assert(type(BFL:GetModule("QuickJoin")) == "table", 
					"QuickJoin module should exist on Retail")
			end
		end,
	})
	...
	TS:RegisterTest("issues", "Issue42_RaidTab_RightClick", {
		...
		action = function(V)
			...
			if not mainFrame then V:Skip("BetterFriendsFrame not found") return end
			
			-- Just verify the raid frame reference exists
			local raidFrame = mainFrame.RaidFrame
			V:Assert(raidFrame ~= nil or BFL.RaidFrame ~= nil, 
				"Raid frame should exist")
		end,
	})
	...
	TS:RegisterTest("issues", "SettingsPersistence_Critical", {
		...
		action = function(V)
			...
			for _, setting in ipairs(criticalSettings) do
				...
				V:Assert(value ~= "ERROR", 
					setting .. " should not return error state")
			end
		end,
	})
		...
		action = function(V)
			...
			for localeName, _ in pairs(BFL.Locales) do
				...
				for _, key in ipairs(requiredKeys) do
					...
					V:Assert(value ~= nil and value ~= "",
						"Missing localization key in " .. localeName .. ": " .. key)
				end
			end
...
function TestSuite:HandlePerfyCommand(args)
	local trimmed = args and args:trim() or ""
...
function TestSuite:RunPerfyStress(durationSeconds)
	...
	context.actions = {
		function() EnsureTab(friendsTab) end,
		function() if FriendsList and FriendsList.RenderDisplay then FriendsList:RenderDisplay(true) end end,
		ScrollFriendsList,
		ToggleNextGroup,
		ApplyNextFilter,
		ApplyNextSort,
		function() if quickJoinTab then EnsureTab(quickJoinTab) end if QuickJoin and QuickJoin.Update then QuickJoin:Update(true) end end,
		function() EnsureTab(raidTab) if RaidFrame and RaidFrame.UpdateGroupLayout then RaidFrame:UpdateGroupLayout() end end,
		function()
			if BetterFriendsFrame and BetterFriendsFrame:IsShown() then
	...
	self.Reporter:Info(string.format(
	...
	self.perfyStressTicker = C_Timer.NewTicker(0.25, function()
		...
		if action then
			...
			if not ok then
				self.Reporter:Warn(string.format(
...
function TestSuite:HandleBenchmarkCommand(args)
	...
	elseif cmd == "trend" then
		...
		for i, entry in ipairs(trend) do
			...
			print(string.format("  %2d. %s - %s%.2fms|r", 
				i, date("%m-%d", entry.timestamp), statusColor, entry.mean))
		end
