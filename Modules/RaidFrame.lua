...
function RaidFrame:UpdateControlPanelLayout()
	...
	if controlPanel.EveryoneAssistCheckbox then
		...
		controlPanel.EveryoneAssistCheckbox:SetPoint(
			"TOPLEFT",
			controlPanel,
			"TOPLEFT",
			checkboxStartX,
...
function RaidFrame:UpdateSecureAttributesForButton(button)
	if not button then
		return
	end
	if InCombatLockdown() then
	...
	-- CRITICAL: Left Click (type1) MUST be nil for Drag & Drop
...
-- Called from OnClick when modifier+button detected
-- Must be called from hardware event to avoid ADDON_ACTION_FORBIDDEN
function RaidFrame:HandleShortcutClick(button, clickedButton, modifier)
	if InCombatLockdown() then
		UIErrorsFrame:AddMessage("Cannot use shortcuts in combat", 1.0, 0.1, 0.1, 1.0)
		return false
	end
	...
	-- Check if this modifier+button combination matches a shortcut
	for actionKey, data in pairs(shortcuts) do
		if DB:Get("raidShortcutEnabled_" .. actionKey, true) then
...
function RaidFrame:UpdateSecureAttributes_OLD_DISABLED()
	...
	for _, mod in ipairs({ "", "shift-", "ctrl-", "alt-", "shift-ctrl-", "shift-alt-", "ctrl-alt-" }) do
		for i = 1, 5 do
			...
			proxy:SetAttribute(mod .. "macrotext" .. i, nil)
		end
	end
		...
		if DB:Get("raidShortcutEnabled_" .. actionKey, true) then
			...
			if button then
				...
				elseif actionKey == "lead" then
					proxy:SetAttribute(modPrefix .. "type" .. btnID, "macro")
					-- Use BFL_RaidFrame_SecureProxy global to get the unit from the proxy itself
					proxy:SetAttribute(
						modPrefix .. "macrotext" .. btnID,
						"/run BFL:GetModule('RaidFrame'):PerformCustomAction('lead', BFL_RaidFrame_SecureProxy:GetAttribute('unit'))"
					)
					secureButtons[button] = true
				elseif actionKey == "promote" then
					proxy:SetAttribute(modPrefix .. "type" .. btnID, "macro")
					proxy:SetAttribute(
						modPrefix .. "macrotext" .. btnID,
						"/run BFL:GetModule('RaidFrame'):PerformCustomAction('promote', BFL_RaidFrame_SecureProxy:GetAttribute('unit'))"
					)
					secureButtons[button] = true
				end
			end
	...
	-- CRITICAL DOCUMENTATION (DO NOT CHANGE):
	-- We MUST NOT add "LeftButton" to PassThrough buttons.
	-- 1. The Proxy must Intercept LeftButton to handle Drag & Drop via OnDragStart (Delegation).
	-- 2. If we PassThrough, the Proxy hides immediately on mouse move (drag), causing Drag Stop.
	-- 3. We use a "Locking" strategy: Proxy sets self.isDragging=true to prevent hiding during drag.
	-- 4. Left Click must be "Free" (No Attribute Action) to ensure Drag works reliably. Targeting is handled via PostClick (Out of Combat).
	local pass = {}
	-- if not secureButtons["LeftButton"] then table.insert(pass, "LeftButton") end -- NEVER PASS THROUGH LEFTBUTTON.

	-- Ensure "Free" Left Click (No Attribute)
	-- User Request: "Linksklick hat keine feste Aktion" (Left click has no fixed action)
	if not proxy:GetAttribute("type1") then
		proxy:SetAttribute("type1", nil) -- Explicitly nil to confirm no action
	end
...
function RaidFrame:UpdateMemberButton(button, memberData)
	...
	if
		combatRole
		and combatRole ~= "NONE"
		and (combatRole == "TANK" or combatRole == "HEALER" or combatRole == "DAMAGER")
	then
		SetRoleIconTexture(button.RoleIcon, combatRole)
		button.RoleIcon:Show()
	else
		button.RoleIcon:Hide()
	end
...
function RaidFrame:UpdateSavedInstances()
	...
	for i = 1, numSaved do
		local name, id, reset, difficulty, locked, extended, instanceIDMostSig, isRaid, maxPlayers, difficultyName, numEncounters, encounterProgress =
...
function RaidFrame:OnCombatStart()
	...
	if BetterRaidFrame_SelectedPlayers and #BetterRaidFrame_SelectedPlayers > 0 then
		...
		UIErrorsFrame:AddMessage(L.MSG_MULTI_SELECTION_CLEARED, 1.0, 0.8, 0.0, 1.0)
	end
end
...
function RaidFrame:ProcessMockDynamicUpdate()
	if not self.mockEnabled or not self.mockConfig.dynamicUpdates then
		return
	end
...
SlashCmdList["BFLRAIDFRAME"] = function(msg)
	...
	elseif cmd == "config" then
		...
		if setting == "dynamic" then
			...
			BFL:DebugPrint(
				"|cff00ff00BFL RaidFrame:|r "
					.. string.format(BFL.L.RAID_DYN_UPDATES, RaidFrame.mockConfig.dynamicUpdates and "ON" or "OFF")
			)
		elseif setting == "interval" then
			...
			BFL:DebugPrint(
				"|cff00ff00BFL RaidFrame:|r "
					.. string.format(BFL.L.RAID_UPDATE_INTERVAL, RaidFrame.mockConfig.updateInterval)
			)
		else
			...
			BFL:DebugPrint(
				string.format(BFL.L.RAID_DYN_UPDATES_STATUS, RaidFrame.mockConfig.dynamicUpdates and "ON" or "OFF")
			)
