...
function MenuSystem:ShowGuildMemberMenu(button, member)
	...
	if GuildActions and GuildActions.ShowMemberMenu and GuildActions:ShowMemberMenu(button, member, "BFL_GuildMemberDropdown") then
		return true
	end

	if BFL.DebugPrint then
		BFL:DebugPrint("Guild member menu unavailable: GuildActions or menu API missing")
	end
	return false
end
