...
function StateValidator:CaptureModuleState(moduleName, module)
    ...
    elseif moduleName == "ActivityTracker" then
        state.trackedActivities = module.activities and self:CountTableEntries(module.activities) or 0
...
function StateValidator:CompareDatabaseStates(oldDB, newDB)
    ...
    local fieldsToCompare = {
        "groupCount", "groupAssignmentCount", "noteCount",
        "hasSettings", "hasColumnWidths", "hasActivityData",
        "hasFavorites", "testScenarioCount",
    }
...
function StateValidator:CompareUIStates(oldUI, newUI)
    ...
    local fieldsToCompare = {
        "mainFrameVisible", "mainFrameWidth", "mainFrameHeight",
...
function StateValidator:PrintSnapshot(snapshot)
    ...
    if snapshot.ui then
        ...
        print("  Frame Size: " .. math.floor(snapshot.ui.mainFrameWidth) .. "x" .. math.floor(snapshot.ui.mainFrameHeight))
    end
end
...
function StateValidator:PrintValidation(result)
    ...
    if result.moduleResults then
        ...
        print(string.format("  Modules: %d passed, %d failed (of %d)", 
            result.passedModules, result.failedModules, result.totalModules))
        print(string.format("  Issues: %d errors, %d warnings", 
    ...
    if #report.potentialRegressions > 0 then
        ...
        for _, item in ipairs(report.potentialRegressions) do
            print(string.format("  * [%s] %s: %s -> %s", 
                item.module, item.change.field, item.change.old, item.change.new))
        end
    else
    ...
    if status.snapshotCount > 0 then
        ...
        for i = 1, status.snapshotCount do
            ...
            if snapshot then
                ...
                print(string.format("  %d. %s [%.1fs ago]%s", 
                    i, snapshot.label, GetTime() - snapshot.timestamp, marker))
            end
        end
    end
end
