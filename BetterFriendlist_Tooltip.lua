...
local function ShowRichPresenceOnly(client, wowProjectID, faction, realmID, areaName)
	...
	if (client ~= BNET_CLIENT_WOW) or (wowProjectID ~= WOW_PROJECT_ID) then
		...
		return true
	elseif
		(WOW_PROJECT_ID == WOW_PROJECT_CLASSIC) and ((faction ~= playerFactionGroup) or (realmID ~= playerRealmID))
	then
...
local function CanCooperateWithGameAccount(accountInfo)
	...
	return gameAccountInfo.factionName == playerFactionGroup and gameAccountInfo.realmID and gameAccountInfo.realmID > 0
end
	...
	if friendUID then
		...
		if ActivityTracker then
			...
			if lastActivity then
				...
				local activityText = "|cff80c0ff"
					.. L.TOOLTIP_LAST_CONTACT
					.. "|r "
					.. (L.TOOLTIP_AGO_PREFIX or "")
					.. SecondsToTime(timeSinceActivity)
					.. (L.TOOLTIP_AGO or "")
...
function BetterFriendsList_Button_OnEnter(self)
	...
	if BFL.IsClassic and not friendData._isMock then
		...
		-- [STREAMER MODE CHECK] Override Real ID in header after Blizzard populated the tooltip
		...
		if accountInfo then
			...
			if accountInfo.gameAccountInfo and accountInfo.gameAccountInfo.gameAccountID then
				...
				then
					...
					anchor = FriendsFrameTooltip_SetLine(
						FriendsTooltipGameAccount1Info,
						nil,
					...
					if CanCooperateWithGameAccount(accountInfo) then
						...
						text = string.format(
							FRIENDS_TOOLTIP_WOW_TOON_TEMPLATE,
							accountInfo.gameAccountInfo.characterName .. CANNOT_COOPERATE_LABEL,
							accountInfo.gameAccountInfo.characterLevel,
							raceName,
							className
						)
				...
				if accountInfo.customMessageTime and not HasTimePassed(accountInfo.customMessageTime, 31536000) then
					customMessage = customMessage
						...
						.. string.format(
							...
							FriendsFrame_GetLastOnline(accountInfo.customMessageTime) .. FONT_COLOR_CODE_CLOSE
						)
				end
			...
			if accountInfo.gameAccountInfo and accountInfo.gameAccountInfo.isOnline then
				...
				elseif accountInfo.lastOnlineTime and accountInfo.lastOnlineTime > 0 then
					...
					if timeDiff >= SECONDS_PER_YEAR then
						...
						local lastOnline = FriendsFrame_GetLastOnline
								and FriendsFrame_GetLastOnline(accountInfo.lastOnlineTime)
							or "Unknown"
	...
	elseif fakeButton.buttonType == FRIENDS_BUTTON_TYPE_WOW then
		...
		if info then
			...
			if info.connected then
				FriendsTooltipHeader:SetTextColor(
					FRIENDS_WOW_NAME_COLOR.r,
					FRIENDS_WOW_NAME_COLOR.g,
	...
	if numGameAccounts > 1 then
		...
		for i = 1, numGameAccounts do
			...
			if
				gameAccountInfo
				and not gameAccountInfo.hasFocus
				and (gameAccountInfo.clientProgram ~= BNET_CLIENT_APP)
				and (gameAccountInfo.clientProgram ~= BNET_CLIENT_CLNT)
			then
				...
				if
					...
					and hasTitleIconTexture
					and C_Texture.IsTitleIconTextureReady(gameAccountInfo.clientProgram, Enum.TitleIconVersion.Small)
				then
					C_Texture.GetTitleIconTexture(
						gameAccountInfo.clientProgram,
						Enum.TitleIconVersion.Small,
				...
				if
					(gameAccountInfo.clientProgram == BNET_CLIENT_WOW)
					and (gameAccountInfo.wowProjectID == WOW_PROJECT_ID)
				then
					if
						(gameAccountInfo.realmName == playerRealmName)
						and (gameAccountInfo.factionName == playerFactionGroup)
					then
						text = text
							.. string.format(
								FRIENDS_TOOLTIP_WOW_TOON_TEMPLATE,
						...
						text = text
							.. string.format(
								FRIENDS_TOOLTIP_WOW_TOON_TEMPLATE,
								gameAccountInfo.characterName .. CANNOT_COOPERATE_LABEL,
								gameAccountInfo.characterLevel,
								raceName,
								className
							)
	...
	for i = gameAccountIndex + 1, FRIENDS_TOOLTIP_MAX_GAME_ACCOUNTS do
		...
		if characterNameString then
			characterNameString:Hide()
		end
		if gameAccountInfoString then
	...
	if numGameAccounts > FRIENDS_TOOLTIP_MAX_GAME_ACCOUNTS then
		FriendsFrameTooltip_SetLine(
			FriendsTooltipGameAccountMany,
			nil,
			string.format(FRIENDS_TOOLTIP_TOO_MANY_CHARACTERS, numGameAccounts - FRIENDS_TOOLTIP_MAX_GAME_ACCOUNTS),
			0
		)
	else
